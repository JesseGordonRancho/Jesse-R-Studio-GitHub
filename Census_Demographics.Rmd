...
district_colnames <- c("District_Code", "Pct1_Name", "Pct2_Name", "Pct3_Name","District_White", "District_Non_White", "District_Size")

district_info <- as_tibble(data.frame(matrix(nrow=0,ncol=length(district_colnames)))) 

colnames(district_info) <- district_colnames
district_info$District_Code   <-as.integer(district_info$District_Code)
  ...
  for (Pct2_input in 1:nrow(pct1_contiguity)) {  
    ...
    for (Pct3_input in 1:nrow(pct2_contiguity)) {  
      ...
      if (pct1_name != pct2_name & pct1_name != pct3_name) {
        district_info <- district_info %>% add_row("Pct1_Name" = pct1_name, "Pct2_Name" = pct2_name, "Pct3_Name" = pct3_name)
...
district_info <- district_info %>% add_row("Precinct_Code" = adjacent_input, "Precinct_Name" = contiguity_adjacent$Precinct_Name[adjacent_input], "Precinct_Status" = contiguity_adjacent$Precinct_Status[adjacent_input], "Precinct_White" = district_white_size, "Precinct_Non_White" = district_non_white_size)
...
while (district_size < target_district_size) {
  non_core_input <- non_core_input + 1 

  # Check if valid district (in range and not in list already)
  if (contiguity_non_core$Precinct_Name[non_core_input] %in% district_info$Precinct_Name) { # skip if it's in already!
    } else {   # Add in the new precinct to the district
  ...
  district_info <- district_info %>% add_row("Precinct_Code" = adjacent_input, "Precinct_Name" = contiguity_non_core$Precinct_Name[non_core_input], "Precinct_Status" = contiguity_non_core$Precinct_Status[non_core_input], "Precinct_White" = district_white_size, "Precinct_Non_White" = district_non_white_size)
    } # if not %in% district already
  }
} # for (adjacent_input
```
