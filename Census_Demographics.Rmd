...
```{r setup_contiguity}
contiguity_colnames <- c("Main_Precinct_Name", "Contiguous_Precinct_Name", "Contiguous_Precinct_Status","Contiguous_Precinct_White","Contiguous_Precinct_Non_White")
...
contiguity_info$Contiguous_Precinct_Name <-as.character(contiguity_info$Contiguous_Precinct_Name)
contiguity_info$Contiguous_Precinct_Status <-as.character(contiguity_info$Contiguous_Precinct_Status)
contiguity_info$Contiguous_Precinct_White <-as.integer(contiguity_info$Contiguous_Precinct_White)
contiguity_info$Contiguous_Precinct_Non_White <-as.integer(contiguity_info$Contiguous_Precinct_Non_White)
...
while (demographic_input < max_census_cols) {
  ...
  if (contiguity_status == 'Adjacent' || contiguity_status == 'Distant') {
       ...
       contiguity_info <- contiguity_info %>% add_row
         ("Main_Precinct_Name" = precinct_name, 
           "Contiguous_Precinct_Name" = contiguous_name)
...
district_colnames <- c("Precinct_Name", "Precinct_Status", "Precinct_White", "Precinct_Non_White")

district_info <- as_tibble(data.frame(matrix(nrow=0,ncol=length(district_colnames)))) 

colnames(district_info) <- district_colnames
district_info$Precinct_Name   <-as.character(district_info$Precinct_Name)
district_info$Precinct_Status <-as.character(district_info$Precinct_Status)
district_info$Precinct_White  <-as.integer(district_info$Precinct_White)
district_info$Precinct_Non_White <- as.integer(district_info$Precinct_Non_White)

district_non_white_count <- core_non_white_count
district_white_count <- core_white_count
district_count <- district_non_white_count + district_white_count

max_adjacent_cols <- ncol(contiguity_adjacent)
max_non_core_cols <- ncol(contiguity_non_core)

adjacent_input <- 1  # just one for now
district_non_white_count <- district_non_white_count + contiguity_adjacent$Precinct_Non_White[adjacent_input]
district_white_count <- district_white_count + contiguity_adjacent$Precinct_White[adjacent_input]
district_size <- district_non_white_count + district_white_count

district_info <- district_info %>% add_row("Precinct_Name" = "precinct_name", "Precinct_Status" = "Test", "Precinct_White" = district_white_count, "Precinct_Non_White" = district_non_white_count)

district_output <- 1  # Start output with adjacent district
district_info$Precinct_Name[district_output] <- contiguity_adjacent$Precinct_Name[district_input]

non_core_input <- 0  # Start counting

while (district_size < target_district_size) {
  non_core_input <- non_core_input + 1 # Check if valid district (in range)
   
  district_non_white_count <- district_non_white_count + contiguity_non_core$Precinct_Non_White[non_core_input]
  district_white_count <- district_white_count + contiguity_non_core$Precinct_White[non_core_input]
  district_size <- district_non_white_count + district_white_count
  district_output <- district_output + 1
  district_info <- district_info %>% add_row("Precinct_Name" = contiguity_non_core$Precinct_Name[non_core_input], "Precinct_Status" = "Test2", "Precinct_White" = district_white_count, "Precinct_Non_White" = district_non_white_count)
