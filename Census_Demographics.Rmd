...
district_non_white_size <- core_non_white_size
district_white_size <- core_white_size
district_size <- district_non_white_size + district_white_size

max_adjacent_cols <- ncol(contiguity_adjacent)
max_non_core_cols <- ncol(contiguity_non_core)

adjacent_input <- 1  # just one for now
district_non_white_size <- district_non_white_size + contiguity_adjacent$Precinct_Non_White[adjacent_input]
district_white_size <- district_white_size + contiguity_adjacent$Precinct_White[adjacent_input]
district_size <- district_non_white_size + district_white_size

district_info <- district_info %>% add_row("Precinct_Name" = contiguity_adjacent$Precinct_Name[adjacent_input], "Precinct_Status" = contiguity_adjacent$Precinct_Status[adjacent_input], "Precinct_White" = district_white_size, "Precinct_Non_White" = district_non_white_size)

district_output <- 1  # Start output with adjacent district
district_info$Precinct_Name[district_output] <- contiguity_adjacent$Precinct_Name[adjacent_input]
...
while (district_size < target_district_size) {
  ...
  district_non_white_size <- district_non_white_size + contiguity_non_core$Precinct_Non_White[non_core_input]
  district_white_size <- district_white_size + contiguity_non_core$Precinct_White[non_core_input]
  district_size <- district_non_white_size + district_white_size
  district_output <- district_output + 1
  district_info <- district_info %>% add_row("Precinct_Name" = contiguity_non_core$Precinct_Name[non_core_input], "Precinct_Status" = contiguity_non_core$Precinct_Status[non_core_input], "Precinct_White" = district_white_size, "Precinct_Non_White" = district_non_white_size)
